...
int threadNum[NUM_THREADS];//thread ids
...
char queryDir[BUFF_SIZE];//path for results of queries
...
void executeQuery(){
	...
	started2 = NUM_THREADS;
		...
		if(WRITE_TO_FILE){
			char buff[BUFF_SIZE];
			snprintf(buff,sizeof(buff),"%s%d%s",queryDir,qid,".dat");//build the output file name based on query id
...
void init(){
	...
	mkdir(dir,S_IRWXU);
	int i=0;
	for(;i<BUFF_SIZE;queryDir[i++]='\0');
	strcpy(queryDir,dir);
	strcat(queryDir,"/qID_");
